...
function GetResponderInfo() {
   ...
   global $DB_OptOutRedir, $DB_OptInDisplay, $DB_OptOutDisplay, $DB_OptInDisplayRedir;
   ...
   if (mysql_num_rows($DB_result) > 0) {
       ...
       $DB_OptInDisplayRedir = $result_data['OptInDisplayRedir'];
...
function UserIsSubscribed() {
 ...
 $DB_result = mysql_query($query, $DB_LinkID)
  ...
  while ($row = mysql_fetch_object($DB_result)) {
      ...
      if ($Temp_Var == $Email_Address) { $Result_Var = TRUE; }
  }
 return $Result_Var;
}
...
function ResponderPulldown($field) {
    ...
    while ($menu_row = mysql_fetch_assoc($menu_result)) {
	 ...
	 $DB_OptInDisplayRedir = $menu_row['OptInDisplayRedir'];
...
function PrimaryKeyName($table) {
  ...
  while ($i < mysql_num_fields($result)) {
     ...
     if ($meta) {
       if ($meta->primary_key) { $PrimaryID_Num = $i; }
     }
     $i++;
  }
