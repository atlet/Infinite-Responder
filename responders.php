...
if ($Is_Auth = User_Auth()) {
  ...
  elseif ($action == "do_create") {
    ...
    $OptInDisplayRedir = MakeSafe($_REQUEST['OptInDisplayRedir']);
    ...
    $query = "INSERT INTO InfResp_responders (Name, Enabled, ResponderDesc, OwnerEmail, OwnerName, ReplyToEmail, MsgList, OptMethod, OptInRedir, OptOutRedir, OptInDisplay, OptOutDisplay, NotifyOwnerOnSub, OptInDisplayRedir)
              VALUES('$Resp_Name', '$Resp_Enabled', '$Resp_Desc', '$Owner_Email', '$Owner_Name', '$Reply_To', '$Msg_List', '$OptMethod', '$OptInRedir', '$OptOutRedir', '$OptInDisp', '$OptOutDisp', '$NotifyOwner', '$OptInDisplayRedir')";
  ...
  elseif ($action == "do_update") {
    ...
    $query = "UPDATE InfResp_responders 
		  ...
		  OptInDisplayRedir = '$OptInDisplayRedir',
